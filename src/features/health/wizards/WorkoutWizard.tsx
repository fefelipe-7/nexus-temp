    ...
    <WizardShell {...CONFIG} step={step} totalSteps={CONFIG.steps} onBack={() => step > 1 ? setStep(s => s - 1) : onClose()} onNext={handleNext} onCommit={handleCommit} saving={salvando} sucesso={sucesso}>
      {step === 1 && (
        <div className="space-y-4">
          <h3 className="text-xs font-bold text-subtle uppercase font-mono tracking-wider">Passo 1: Identificar Exercício</h3>
          <p className="text-[11px] text-subtle font-medium">Qual atividade física você realizou ou está planejando?</p>
          <div className="space-y-4">
            <div className="space-y-1">
              <label className="text-[10px] font-mono text-subtle uppercase font-semibold">Nome da Atividade</label>
              <input type="text" value={treinoNome} placeholder="Ex: Musculação - Costas & Bíceps" onChange={e => setTreinoNome(e.target.value)} className="w-full text-xs font-medium border border-line bg-card rounded-md p-2.5 focus:outline-hidden" />
            </div>
            <div className="space-y-1">
              <label className="text-[10px] font-mono text-subtle uppercase font-semibold">Tipo do Treino</label>
              <select value={treinoTipo} onChange={e => setTreinoTipo(e.target.value)} className="w-full text-xs bg-card mt-1 border border-line rounded-md p-2">
      ...
      {step === 2 && (
        <div className="space-y-4">
          <h3 className="text-xs font-bold text-subtle uppercase font-mono tracking-wider">Passo 2: Métricas de Tempo</h3>
          <p className="text-[11px] text-subtle font-medium">Determine o tempo de exercício realizado.</p>
      ...
      {step === 3 && (
        <div className="space-y-4">
          <h3 className="text-xs font-bold text-subtle uppercase font-mono tracking-wider">Passo 3: Esforço Percebido</h3>
          <p className="text-[11px] text-subtle font-medium">Qual o nível de dificuldade do esforço investido?</p>
          <div className="bg-muted rounded-xl p-4 border border-line space-y-4 text-center">
