    ...
    <WizardShell {...CONFIG} step={step} totalSteps={CONFIG.steps} onBack={() => step > 1 ? setStep(s => s - 1) : onClose()} onNext={() => setStep(s => s + 1)} onCommit={handleCommit} saving={salvando} sucesso={sucesso}>
      {step === 1 && (
        <div className="space-y-4">
          <h3 className="text-xs font-bold text-subtle uppercase font-mono tracking-wider">Passo 1: Termômetro do Humor</h3>
          <p className="text-[11px] text-subtle font-medium">De forma intuitiva, como está seu ânimo e humor agora?</p>
          <div className="bg-muted rounded-xl p-4 border border-line space-y-4 text-center">
      ...
      {step === 2 && (
        <div className="space-y-4">
          <h3 className="text-xs font-bold text-subtle uppercase font-mono tracking-wider">Passo 2: Pressão Emocional</h3>
          <p className="text-[11px] text-subtle font-medium">Avalie seu grau de estresse e ansiedade.</p>
          <div className="space-y-5 bg-muted rounded-xl p-4 border border-line">
            <div className="space-y-1">
              <div className="flex justify-between text-xs"><span className="font-bold text-subtle">Nível de Estresse</span><span className="font-mono font-bold text-emerald-600">{estresseRating}/10</span></div>
            ...
            <div className="space-y-1">
              <div className="flex justify-between text-xs"><span className="font-bold text-subtle">Nível de Ansiedade</span><span className="font-mono font-bold text-emerald-600">{ansiedadeRating}/10</span></div>
      ...
      {step === 3 && (
        <div className="space-y-4">
          <h3 className="text-xs font-bold text-subtle uppercase font-mono tracking-wider">Passo 3: Nível de Foco & Cognição</h3>
          <p className="text-[11px] text-subtle font-medium">Anote o estado de atenção subjetivo e notas rápidas.</p>
          <div className="space-y-4">
            <div className="bg-muted rounded-xl p-4 border border-line space-y-2">
              <div className="flex justify-between text-xs"><span className="font-bold text-subtle">Atenção e Foco de Trabalho</span><span className="font-mono font-bold text-teal-600">{focoRating}/10</span></div>
            ...
            <input type="text" value={moodNotas} placeholder="Ex: Consegui entregar o projeto antes do prazo..." onChange={e => setMoodNotas(e.target.value)} className="w-full text-xs bg-card border border-line rounded-md p-2 focus:outline-hidden" />
          </div>
        </div>
      )}
      {step === 4 && (
