    ...
    <WizardShell {...CONFIG} step={step} totalSteps={CONFIG.steps} onBack={() => step > 1 ? setStep(s => s - 1) : onClose()} onNext={handleNext} onCommit={handleCommit} saving={salvando} sucesso={sucesso}>
      {step === 1 && (
        <div className="space-y-4">
          <h3 className="text-xs font-bold text-subtle uppercase font-mono tracking-wider">Passo 1: Direcionador Mental</h3>
          <p className="text-[11px] text-subtle font-medium">Selecione uma linha de foco para sua reflexão de hoje.</p>
          <div className="grid grid-cols-1 gap-2.5">
            {prompts.map((p, i) => (
              <button key={i} onClick={() => setDiarioPrompt(p)} className={`text-left text-xs p-3.5 border rounded-lg transition-all cursor-pointer ${diarioPrompt === p ? 'bg-violet-50 border-violet-500 text-violet-700 font-bold' : 'bg-muted hover:bg-neutral-100 text-subtle'}`}>{p}</button>
            ))}
          </div>
        </div>
      )}
      {step === 2 && (
        <div className="space-y-4">
          <h3 className="text-xs font-bold text-subtle uppercase font-mono tracking-wider">Passo 2: Escrever Linhas de Entrada</h3>
          <p className="text-[11px] text-subtle font-medium">Deixe seus sentimentos fluírem para a tela.</p>
          <div className="space-y-2">
            ...
            <textarea value={diarioConteudo} onChange={e => setDiarioConteudo(e.target.value)} placeholder="Comece a digitar suas reflexões..." className="w-full h-40 text-xs text-ink bg-card border border-line rounded-lg p-3 resize-none focus:border-violet-500 focus:outline-hidden" />
            <div className="text-right text-[9px] font-mono text-subtle">{diarioConteudo.length} caracteres</div>
          </div>
        </div>
      )}
      {step === 3 && (
        <div className="space-y-4">
          <h3 className="text-xs font-bold text-subtle uppercase font-mono tracking-wider">Passo 3: Sentimento Dominante</h3>
          <p className="text-[11px] text-subtle font-medium">Qual a cor ou humor que melhor sintoniza este texto?</p>
          <div className="grid grid-cols-2 gap-2">
            {['Grato', 'Inspirado', 'Reflexivo', 'Cansado', 'Ansioso', 'Focado', 'Calmo'].map(mood => (
              <button key={mood} onClick={() => setDiarioEmocao(mood)} className={`text-xs p-3 border rounded-lg text-left transition-all cursor-pointer ${diarioEmocao === mood ? 'bg-violet-100 border-violet-500 text-violet-700 font-bold' : 'bg-muted hover:bg-neutral-100 text-subtle'}`}>🎨 {mood}</button>
            ))}
          </div>
        </div>
      )}
