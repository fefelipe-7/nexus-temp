    ...
    <WizardShell {...CONFIG} step={step} totalSteps={CONFIG.steps} onBack={() => step > 1 ? setStep(s => s - 1) : onClose()} onNext={handleNext} onCommit={handleCommit} saving={salvando} sucesso={sucesso}>
      {step === 1 && (
        <div className="space-y-4">
          ...
          <p className="text-[11px] text-slate font-medium">Selecione um hábito existente ou crie uma nova trilha.</p>
          <div className="space-y-4">
            {habitsList.length > 0 && (
              <div className="space-y-1.5">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {habitsList.map(h => (
                    <button key={h.id} type="button" onClick={() => { setSelectedHabitId(h.id); setNovoHabitoNome(''); }} className={`text-xs p-2.5 border rounded-lg text-left transition-all truncate cursor-pointer ${selectedHabitId === h.id && !novoHabitoNome ? 'bg-pink-100 border-pink-500 text-pink-700 font-bold' : 'bg-surface hover:bg-neutral-100'}`}>🔹 {h.nome}</button>
                  ))}
                </div>
              </div>
            )}
            <div className="pt-2 border-t border-hairline space-y-1">
              ...
              <input type="text" value={novoHabitoNome} placeholder="Ex: Meditação diária, Ler 15 pgs..." onChange={e => { setNovoHabitoNome(e.target.value); setSelectedHabitId(''); }} className="w-full text-xs bg-canvas border border-hairline rounded-md p-2 focus:outline-hidden" />
            </div>
          </div>
        </div>
      )}
      {step === 2 && (
        <div className="space-y-4">
          ...
          <p className="text-[11px] text-slate font-medium">Deseja adicionar notas ou pensamentos sobre esta repetição?</p>
          <textarea value={habitNota} onChange={e => setHabitNota(e.target.value)} placeholder="Ex: Hoje foi mais fácil, fiz logo ao acordar..." className="w-full h-32 text-xs bg-canvas text-charcoal border border-hairline rounded-lg p-3 focus:outline-hidden" />
        </div>
      )}
      {step === 3 && (
        <div className="space-y-4">
          <h3 className="text-xs font-bold text-slate uppercase font-mono tracking-wider">Passo 3: Revisão Rápida</h3>
          <p className="text-[11px] text-slate font-medium">O hábito será registrado para o dia: <strong className="font-mono font-bold">{selectedDate}</strong>.</p>
