  ...
  return (
    <header className="flex justify-between items-start px-0.5">
      <div>
        ...
        <p className="text-xs text-subtle font-medium capitalize mt-0.5">{formattedDate}</p>
      </div>
      <div className="relative">
        <button className="flex items-center gap-1 text-[11px] font-bold text-subtle bg-card border border-line hover:border-subtle px-2.5 py-1.5 rounded-lg active-tap cursor-pointer min-h-[36px]">
          <CalendarDays className="w-3.5 h-3.5 text-faint" />
          <span>Dia</span>
          <input
            type="date"
            value={selectedDate}
