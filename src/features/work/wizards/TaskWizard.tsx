    ...
    <WizardShell {...CONFIG} step={step} totalSteps={CONFIG.steps} onBack={() => step > 1 ? setStep(s => s - 1) : onClose()} onNext={handleNext} onCommit={handleCommit} saving={salvando} sucesso={sucesso}>
      {step === 1 && (
        <div className="space-y-4">
          <h3 className="text-xs font-bold text-subtle uppercase font-mono tracking-wider">Passo 1: Missão Estratégica</h3>
          <p className="text-[11px] text-subtle font-medium">Insira o objetivo principal que precisa ser executado.</p>
          <div className="space-y-4">
            <div className="space-y-1">
              <label className="text-[10px] font-mono text-subtle uppercase font-semibold">Nome da Missão</label>
              <input type="text" value={tarefaNome} placeholder="Ex: Preparar documentação do Nexus V2" onChange={e => setTarefaNome(e.target.value)} className="w-full text-xs font-medium bg-card border border-line rounded-md p-2.5 focus:outline-hidden" />
            </div>
            <div className="space-y-1">
              <label className="text-[10px] font-mono text-subtle uppercase font-semibold">Instrução ou Detalhe Interno (Opcional)</label>
              <input type="text" value={tarefaDet} placeholder="Ex: Revisar tabelas de dados de biohacking" onChange={e => setTarefaDet(e.target.value)} className="w-full text-xs bg-card border border-line rounded-md p-2 focus:outline-hidden" />
            </div>
          </div>
        </div>
      )}
      {step === 2 && (
        <div className="space-y-4">
          <h3 className="text-xs font-bold text-subtle uppercase font-mono tracking-wider">Passo 2: Urgência e Prioridade</h3>
          <p className="text-[11px] text-subtle font-medium">Organize o impacto dessa meta.</p>
          <div className="grid grid-cols-1 gap-2 bg-muted rounded-xl p-4 border border-line">
            {(['baixa', 'media', 'alta'] as const).map(prio => (
              <button key={prio} onClick={() => setTarefaPrio(prio)} className={`text-xs p-3.5 border rounded-lg transition-all capitalize font-bold flex items-center justify-between cursor-pointer ${tarefaPrio === prio ? 'bg-sky-500 text-white border-sky-500' : 'bg-card text-ink'}`}>
      ...
      {step === 3 && (
        <div className="space-y-4">
          <h3 className="text-xs font-bold text-subtle uppercase font-mono tracking-wider">Passo 3: Estimativa de Horas</h3>
          <p className="text-[11px] text-subtle font-medium">Quantas horas de foco puro são necessárias?</p>
          <div className="bg-muted rounded-xl p-4 border border-line text-center space-y-4">
