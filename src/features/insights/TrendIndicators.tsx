...
export function TrendIndicators() {
  ...
  return (
    <div className="space-y-2 pt-1.5">
      ...
      <div className="bg-white border border-[#E3E0D8] rounded-[22px] p-4.5 space-y-3.5 shadow-none">
        {trends.map((t, i) => (
          <div key={i} className={`flex items-center justify-between text-xs py-0.5 ${i > 0 ? 'border-t border-[#F0EFEB] pt-2.5' : ''}`}>
            <div className="flex items-center gap-2">
              <span className={`w-1.5 h-1.5 rounded-full ${t.dotColor}`} />
              <span className="text-[#20201D] font-semibold text-[12px]">{t.label}</span>
            </div>
