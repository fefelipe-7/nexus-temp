...
import { cn } from '../../lib/utils'; // adjusted to relative path to avoid path alias issues
...
export function Button({
  ...
  const Comp = (asChild ? SlotPrimitive : 'button') as any;
  return (
    <Comp
      data-slot="button"
      className={cn(
...
interface ButtonArrowProps extends React.HTMLAttributes<HTMLOrSVGElement> {
  icon?: any; // Allows passing any Lucide icon
  className?: string;
}

export function ButtonArrow({ icon: Icon = ChevronDown, className, ...props }: ButtonArrowProps) {
